        ...
        success: function (data) {
            ...
            $(".token").val(result.hash);
                ...
                table.row.add( [
                    ...
                    result.fecha,
                    result.nombre,
                    '<button type="button" id="user_'+result.planta_id+'" class="btn btn-danger btn-flat" onclick="showModalDelete('+result.planta_id+', '+"'"+result.nombre+"'"+');" style="margin-right: 3px;"><i class="fa fa-close"></i></button>'
                    +'<button type="button" id="userEdit_'+result.planta_id+'" class="btn btn-primary btn-flat" onclick="showModalUpdate('+result.planta_id+');" data-toggle="modal" data-target="#Editar"><i class="fa fa-edit"></i></button>'
                ] ).draw( false );
...
function userDelete() {
    ...
    $.ajax({
        type: 'post',
        url: './Cl_Plantas/userDelete',
        data: {planta_id: tr, csrf_ars: token},
        success:function (data) {
            var result = JSON.parse(data);
            $(".token").val(result.hash);
            /**
...
function showModalUpdate(id){
    $.ajax({
        type: 'get',
        url: './Cl_Plantas/plantaGet',
        data: {planta_id: id},
        success: function (data) {
            var result = JSON.parse(data);
            $("#EditarUsuarios").find("#id_editar").val(id);
            $("#EditarUsuarios").find("input[name='nombre']").val(result.plantas.nombre);
            $("#Editar").modal("show");
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            table.row.add( [
                ...
                result.nombre,
                '<button type="button" id="user_'+result.planta_id+'" class="btn btn-danger btn-flat" onclick="showModalDelete('+result.planta_id+', '+"'"+result.nombre+"'"+');" style="margin-right: 3px;"><i class="fa fa-close"></i></button>'
                +'<button type="button" id="userEdit_'+result.planta_id+'" class="btn btn-primary btn-flat" onclick="showModalUpdate('+result.planta_id+');"><i class="fa fa-edit"></i></button>'
            ] ).draw( false );
