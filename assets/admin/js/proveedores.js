...
$("#RegistroUsuarios").submit(function(e){
    ...
    $.ajax({
        type: 'post',
        url: './Cl_Proveedores/proveedorAdd',
        data: formData,
        ...
        success: function (data) {
            ...
            $(".token").val(result.hash);
                ...
                table.row.add( [
                    ...
                    result.fecha,
                    result.nombre,
                    '<button type="button" id="user_'+result.proveedor_id+'" class="btn btn-danger btn-flat" onclick="showModalDelete('+result.proveedor_id+', '+"'"+result.nombre+"'"+');" style="margin-right: 3px;"><i class="fa fa-close"></i></button>'
                    +'<button type="button" id="userEdit_'+result.proveedor_id+'" class="btn btn-primary btn-flat" onclick="showModalUpdate('+result.proveedor_id+');" data-toggle="modal" data-target="#Editar"><i class="fa fa-edit"></i></button>'
                ] ).draw( false );
...
function userDelete() {
    ...
    $.ajax({
        type: 'post',
        url: './Cl_Proveedores/userDelete',
        data: {proveedor_id: tr, csrf_ars: token},
        success:function (data) {
            var result = JSON.parse(data);
            $(".token").val(result.hash);
            /**
...
function showModalUpdate(id){
    $.ajax({
        type: 'get',
        url: './Cl_Proveedores/proveedorGet',
        data: {proveedor_id: id},
        success: function (data) {
            var result = JSON.parse(data);
            $("#EditarUsuarios").find("#id_editar").val(id);
            $("#EditarUsuarios").find("input[name='nombre']").val(result.proveedores.nombre);
            $("#Editar").modal("show");
        }
    });
}
...
$("#EditarUsuarios").submit(function (e) {
    ...
    $.ajax({
        type: 'post',
        url: './Cl_Proveedores/proveedorUpdate',
        data: formData,
        processData: false,
        contentType: false,
        success: function (data) {
            ...
            table.row.add( [
                ...
                result.nombre,
                '<button type="button" id="user_'+result.proveedor_id+'" class="btn btn-danger btn-flat" onclick="showModalDelete('+result.proveedor_id+', '+"'"+result.nombre+"'"+');" style="margin-right: 3px;"><i class="fa fa-close"></i></button>'
                +'<button type="button" id="userEdit_'+result.proveedor_id+'" class="btn btn-primary btn-flat" onclick="showModalUpdate('+result.proveedor_id+');"><i class="fa fa-edit"></i></button>'
            ] ).draw( false );
